...
import color.kidpaint.com.kidpaintcolor.command.implementation.BaseCommand;
import color.kidpaint.com.kidpaintcolor.command.implementation.ResizeCommand;
...
public class ResizeTool extends BaseToolWithRectangleShape {
	...
	public ResizeTool(Context context, ToolType toolType) {
		...
		mBoxHeight = AnimalsColoringApplication.drawingSurface.getBitmapHeight();
		mBoxWidth = AnimalsColoringApplication.drawingSurface.getBitmapWidth();
	...
	private void resetScaleAndTranslation() {
		...
		float zoomFactor = AnimalsColoringApplication.perspective
				.getScaleForCenterBitmap() * START_ZOOM_FACTOR;
		AnimalsColoringApplication.perspective.setScale(zoomFactor);
	}
	...
	private void initialiseResizingState() {
		...
		mResizeBoundWidthXLeft = AnimalsColoringApplication.drawingSurface
				.getBitmapWidth();
		mResizeBoundHeightYTop = AnimalsColoringApplication.drawingSurface
				.getBitmapHeight();
		mIntermediateResizeBoundWidthXLeft = 0;
		mIntermediateResizeBoundWidthXRight = AnimalsColoringApplication.drawingSurface
				.getBitmapWidth();
		mIntermediateResizeBoundHeightYTop = 0;
		mIntermediateResizeBoundHeightYBottom = AnimalsColoringApplication.drawingSurface
				.getBitmapHeight();
		resetScaleAndTranslation();
	}
	...
	private boolean areResizeBordersValid() {
		...
		if (mResizeBoundWidthXLeft >= AnimalsColoringApplication.drawingSurface.getBitmapWidth() ||
				mResizeBoundWidthXRight < 0 || mResizeBoundHeightYBottom < 0 ||
				mResizeBoundHeightYTop >= AnimalsColoringApplication.drawingSurface.getBitmapHeight()) {
			return false;
		}
		if (mResizeBoundWidthXLeft == 0 && mResizeBoundHeightYTop == 0 &&
				mResizeBoundWidthXRight == AnimalsColoringApplication.drawingSurface.getBitmapWidth() - 1 &&
				mResizeBoundHeightYBottom == AnimalsColoringApplication.drawingSurface.getBitmapHeight() - 1) {
			return false;
		}
		...
		if (data instanceof BaseCommand.NOTIFY_STATES) {
			if (BaseCommand.NOTIFY_STATES.COMMAND_DONE == data
				...
				mResizeBoundWidthXRight = Float
						.valueOf(AnimalsColoringApplication.drawingSurface
								.getBitmapWidth() - 1);
	...
	protected class FindCroppingCoordinatesAsyncTask extends
		...
		FindCroppingCoordinatesAsyncTask() {
			...
			mBitmapHeight = AnimalsColoringApplication.drawingSurface
					.getBitmapHeight();
		...
		protected Void doInBackground(Void... arg0) {
			if (AnimalsColoringApplication.drawingSurface
		...
		private void getBitmapPixelsLineWidth(int[] bitmapPixelsArray,
			...
			AnimalsColoringApplication.drawingSurface.getPixels(bitmapPixelsArray, 0,
