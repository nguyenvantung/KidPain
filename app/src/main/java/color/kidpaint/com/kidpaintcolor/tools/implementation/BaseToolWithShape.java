...
import color.kidpaint.com.kidpaintcolor.tools.ToolWithShape;

public abstract class BaseToolWithShape extends BaseTool implements
		ToolWithShape {

	protected int mPrimaryShapeColor = AnimalsColoringApplication.applicationContext
			.getResources().getColor(R.color.rectangle_primary_color);
	protected int mSecondaryShapeColor = AnimalsColoringApplication.applicationContext
	...
	protected float getInverselyProportionalSizeForZoom(float defaultSize) {
		float displayScale = mContext.getResources().getDisplayMetrics().density;
		float applicationScale = AnimalsColoringApplication.perspective.getScale();
	...
	public Point getAutoScrollDirection(float pointX, float pointY,
		...
		int deltaX = 0;
		int deltaY = 0;
		PointF surfaceToolPosition = AnimalsColoringApplication.perspective
