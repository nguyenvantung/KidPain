...
import color.kidpaint.com.kidpaintcolor.command.UndoRedoManager;
...
public class TopBar extends Observable implements OnTouchListener {
	...
	private static final int SWITCH_TOOL_TOAST_Y_OFFSET = (int) 50 + 25;
	...
	protected ColorActivity mainActivity;

	private Toast mToolNameToast;
	private boolean mUndoDisabled;
	private boolean mRedoDisabled;

	public TopBar(ColorActivity mainActivity, boolean openedFromCatroid) {
		this.mainActivity = mainActivity;
		mCurrentTool = new DrawTool(mainActivity, ToolType.BRUSH);
		AnimalsColoringApplication.currentTool = mCurrentTool;
	...
	public void setTool(Tool tool) {
		...
		if (((tool.getToolType() == ToolType.MOVE) || (tool.getToolType() == ToolType.ZOOM))
			...
			setToolSwitchBackground(mPreviousTool
					.getAttributeButtonResource(Tool.ToolButtonIDs.BUTTON_ID_TOOL));
